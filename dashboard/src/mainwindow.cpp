...
MainWindow::MainWindow(QWidget* parent)
  ...
  connect(
      ui->agentList, &QListWidget::itemClicked, this,
      [this](QListWidgetItem* item) {
        const QString clicked = item->data(Qt::UserRole).toString();
        ui->agentNameLabel->setText(item->data(Qt::UserRole + 1).toString());
  ...
  connect(m_client, &ServerClient::agentReceived, this,
          [this](const QString& id, const QString& name, const QString& os,
                 const QString& ip,
                 bool online) { addAgentItem(id, name, os, ip, online); });
  ...
  const QString serverHost =
      QString::fromStdString(EnvHelper::resolveString("DASHBOARD_SERVER_HOST"));
  const std::uint16_t serverPort = EnvHelper::resolvePort();
  qDebug() << "server host value = " << serverHost;
  qDebug() << "server port value = " << serverPort << "\n";
...
void MainWindow::buildContentArea() {
  ...
  m_diskChart = createChart(
      "Disk I/O — sda",
      {{20, 35, 30, 45, 40, 55, 35, 50, 40}, {5, 8, 6, 10, 7, 12, 8, 14, 9}},
      {"read", "write"});
...
void MainWindow::buildStatusBar() {
  ...
  // side
  // ui->statusbar->addWidget(makeLabel("db: PostgreSQL connected",
  // "statusItem")); App version — static, not server-driven.
...
void MainWindow::onMetricsReceived(const QString& target,
  ...
  m_diskHistory.append(
      {diskRead / 1024.0 / 1024.0, diskWrite / 1024.0 / 1024.0});
  m_diskChart->setSeries(m_diskHistory.series());
...
void MainWindow::updateLastSampleLabel() {
  ...
  const qint64 seconds =
      m_lastSampleTime.secsTo(QDateTime::currentDateTimeUtc());
