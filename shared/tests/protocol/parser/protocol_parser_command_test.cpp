...
std::vector<uint8_t> makeCommandPayload(uint16_t id, uint8_t rawType,
...
TEST(ProtocolParserCommand, should_parse_shell_command_when_input_is_valid) {
  ...
  const std::vector<uint8_t> input = makeCommandPayload(
      42, static_cast<uint8_t>(CommandType::SHELL),
...
TEST(ProtocolParserCommand, should_parse_os_info_command_with_empty_data) {
  const std::vector<uint8_t> input =
      makeCommandPayload(7, static_cast<uint8_t>(CommandType::OS_INFO), 0, {});
...
TEST(ProtocolParserCommand, should_reject_payload_shorter_than_fixed_fields) {
  const std::vector<uint8_t> input = {0x00, 0x2A, 0x02, 0x00};
...
TEST(ProtocolParserCommand,
  ...
  const std::vector<uint8_t> input =
      makeCommandPayload(1, static_cast<uint8_t>(CommandType::SHELL), 10,
