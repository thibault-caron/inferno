...
std::vector<uint8_t> makeRegisterPayload(
    ...
    const uint16_t declaredHostnameLen,
    const std::vector<uint8_t>& hostnameBytes) {
  std::vector<uint8_t> out;
  out.reserve(REGISTER_FIXED_BYTES + hostnameBytes.size());
  out.push_back(rawOs);
  out.push_back(rawArch);
  out.push_back(static_cast<uint8_t>((declaredHostnameLen >> 8) & 0xFF));
  out.push_back(static_cast<uint8_t>(declaredHostnameLen & 0xFF));
  out.insert(out.end(), hostnameBytes.begin(), hostnameBytes.end());
  return out;
}

std::vector<uint8_t> bytesFromString(const std::string& string) {
...
TEST(ProtocolParserRegister,
  ...
  const std::vector<uint8_t> input = makeRegisterPayload(
      static_cast<uint8_t>(OSType::LINUX), static_cast<uint8_t>(ArchType::X64),
...
TEST(ProtocolParserRegister, should_reject_empty_hostname) {
  const std::vector<uint8_t> input =
      makeRegisterPayload(static_cast<uint8_t>(OSType::WINDOWS),
                          static_cast<uint8_t>(ArchType::X86), 0, {});
...
TEST(ProtocolParserRegister, should_accept_max_hostname_length) {
  ...
  const std::vector<uint8_t> input = makeRegisterPayload(
      static_cast<uint8_t>(OSType::MAC), static_cast<uint8_t>(ArchType::ARM),
...
TEST(ProtocolParserRegister,
  ...
  const std::vector<uint8_t> input = {static_cast<uint8_t>(OSType::LINUX),
                                      static_cast<uint8_t>(ArchType::X64),
                                      0x00};
...
TEST(ProtocolParserRegister, should_decode_hostname_length_as_big_endian) {
  ...
  const std::vector<uint8_t> input = makeRegisterPayload(
      static_cast<uint8_t>(OSType::WINDOWS),
      static_cast<uint8_t>(ArchType::X86), 300, bytesFromString(hostname));
  const RegisterPayload result = ProtocolParser::parseRegisterPayload(input);
