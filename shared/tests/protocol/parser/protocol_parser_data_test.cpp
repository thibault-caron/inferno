...
std::vector<std::uint8_t> makeDataPayload(
...
TEST(ProtocolParserData, should_parse_data_payload_when_input_is_valid) {
  ...
  const std::vector<std::uint8_t> input =
      makeDataPayload(static_cast<std::uint8_t>(DataType::KEYLOGGER),
...
TEST(ProtocolParserData, should_reject_null_data_size) {
  const std::vector<std::uint8_t> input =
      makeDataPayload(static_cast<std::uint8_t>(DataType::KEYLOGGER), 0, {});
...
TEST(ProtocolParserData, should_reject_unknown_data_subtype) {
  ...
  const std::vector<std::uint8_t> input =
      makeDataPayload(static_cast<std::uint8_t>(DataType::END),
                      static_cast<std::uint16_t>(data.size()),
                      TestHelpers::bytesFromString(data));
...
TEST(ProtocolParserData,
  ...
  const std::vector<std::uint8_t> input =
      makeDataPayload(static_cast<std::uint8_t>(DataType::KEYLOGGER), 10,
