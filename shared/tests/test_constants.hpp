...
constexpr std::uint16_t TCP_SERVER_DOUBLE_START_PORT  = 19873;  // start() called twice
constexpr std::uint16_t TCP_SERVER_OCCUPIED_PORT      = 19876;  // port already taken
constexpr std::uint16_t TCP_SERVER_NOT_STARTED_PORT   = 19877;  // accept before start
constexpr std::uint16_t TCP_SERVER_AGENT_CONNECT_PORT = 19879;  // single agent connects
constexpr std::uint16_t TCP_SERVER_ECHO_PORT          = 19880;  // echo data back
constexpr std::uint16_t TCP_SERVER_REMOTE_ADDR_PORT   = 19881;  // check remote addr/port
